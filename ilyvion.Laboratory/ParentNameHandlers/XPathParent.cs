...
public class XPathParent : ICustomParentNameHandler
{
    public XmlNode? GetBestParentFor(XmlNode node, string xpath, List<XmlNode> allRegisteredNodes)
    {
        ...
        {
            Logger.LogError($"Error resolving XPath {resolvedXPath} for node: \"{node.Name}\". "
                + $"Full node: \"{node.OuterXml}\"; Exception: {e}");
            return null;
        }
        if (matches.Count == 0)
        {
            Logger.LogError($"No matches found for ilyvion.XPathParent using provided XPath fragment "
        ...
        {
            Logger.LogError($"Multiple matches found for ilyvion.XPathParent using provided XPath fragment "
                + $"'{xpath}' for node: \"{node.Name}\". "
                + $"Resolved XPath: '{resolvedXPath}' "
